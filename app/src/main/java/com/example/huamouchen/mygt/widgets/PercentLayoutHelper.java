        ...
        for (int i = 0, N = mHost.getChildCount(); i < N; i++) {
            ...
            if (params instanceof PercentLayoutParams) {
                ...
                if (info != null) {
                    ...
                    //drawable
                    supportDrawableSize(widthHint, heightHint, view, info);
                    //padding
                    supportPadding(widthHint, heightHint, view, info);
//                    for starBar
                    supportStarBar(widthHint, heightHint, view, info);
    ...
    private void supportStarBar(int widthHint, int heightHint, View view, PercentLayoutInfo info){
        //StarBar percent support
        if (view instanceof StarBar) {
            PercentLayoutInfo.PercentVal starBarSizePercent = info.starBarSizePercent;
            if (starBarSizePercent != null) {
                int base = starBarSizePercent.isBaseWidth ? widthHint : heightHint;
                float textSize = (int) (base * starBarSizePercent.percent);
                ((StarBar) view).setStarSize(Math.round(textSize));
            }
        }
    }
    ...
    private void supportDrawableSize(int widthHint, int heightHint, View view, PercentLayoutInfo info) {
        //drawableSize percent support
        if (view instanceof TextView) {
            PercentLayoutInfo.PercentVal drawableSizePercent = info.drawableSizePercent;
            if (drawableSizePercent != null) {
                int base = drawableSizePercent.isBaseWidth ? widthHint : heightHint;
                float drawableSize = (int) (base * drawableSizePercent.percent);
                TextView tv = (TextView) view;
                Drawable[] d = tv.getCompoundDrawables();
                for (int i = 0; i < d.length; i++) {
                    if (d[i] != null)
                        d[i].setBounds(0, 0, Math.round(drawableSize), Math.round(drawableSize));
                }
                tv.setCompoundDrawables(d[0], d[1], d[2], d[3]);
            }
        }
    }
    ...
    public static PercentLayoutInfo getPercentLayoutInfo(Context context,
        ...
        //drawableSizePercent
        sizeStr = array.getString(R.styleable.PercentLayout_Layout_layout_drawableSizePercent);
        percentVal = getPercentVal(sizeStr, false);
        if (percentVal != null) {
            if (Log.isLoggable(TAG, Log.VERBOSE)) {
                Log.v(TAG, "percent drawable size: " + percentVal.percent);
            }
            info = checkForInfoExists(info);
            info.drawableSizePercent = percentVal;
        }
        //padding
        ...
        sizeStr = array.getString(R.styleable.PercentLayout_Layout_layout_starBarSizePercent);
        percentVal = getPercentVal(sizeStr, false);
        if (percentVal != null) {
            if (Log.isLoggable(TAG, Log.VERBOSE)) {
                Log.v(TAG, "percent starBar starSize: " + percentVal.percent);
            }
            info = checkForInfoExists(info);
            info.starBarSizePercent = percentVal;
        }
    ...
    public static class PercentLayoutInfo {
        ...
        //update in 2015/08/11
        public PercentVal drawableSizePercent;

        //update in 2016/04/25
        ...
        // update in 2017/09/12
        public PercentVal starBarSizePercent;
        ...
        public String toString() {
            return String.format(Locale.getDefault(),"PercentLayoutInformation width: %f height %f, margins (%f, %f, "
